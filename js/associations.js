async function searchAssociations(query = '', mapCenter = null, distance = null) {
    ...
    if (query != '' && mapCenter) {
        request += `where=position='Active' and (object like '${query}' or title like '${query}') and within_distance(geo_point_2d, GEOM'POINT(${lon} ${lat})', ${distance}${unit})&`
    }
    if (mapCenter && query == '') {
        request += `where= position= 'Active' and within_distance(geo_point_2d, GEOM'POINT(${lon} ${lat})', ${distance}${unit})&`;
    }
    request += `order_by website DESC &limit=100`;
    ...
    associationsList.innerHTML = '';
        associations.forEach(association => {
            ...
            if (association.object)
                listItem.innerHTML += `<p class='object-text'>${association.object}</p>`
            ...
            if (association.website)
                listItem.innerHTML += `<a href='${association.website}'>${association.website}</a>`
